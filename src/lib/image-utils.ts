 ...
 */
export function getProductImages(baseName: string, count: number = 3): string[] {
  const images: string[] = [];
 ...
 */
export function getProductImagesByName(productName: keyof typeof PRODUCT_IMAGE_MAP): string[] {
  const baseName = PRODUCT_IMAGE_MAP[productName];
  return getProductImages(baseName);
}

/**
 * Verifica si una imagen existe (para uso en el servidor)
 * Esta función puede expandirse para verificación real de archivos
 */
 ...
 */
export function generatePlaceholderSVG(text: string, width: number = 800, height: number = 800): string {
 ...
 */
export function getImageSizes(context: 'card' | 'gallery' | 'thumbnail' | 'hero'): string {
  const sizeMap = {
